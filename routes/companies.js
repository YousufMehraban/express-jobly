...
router.get("/", async function (req, res, next) {
  // geting all companies data and filtering by number of employees and name. 
  // filter data is taken from query.
  ...
  try {
    ...
    // validating the filter terms by jsonWebToken. if not validated, throw error.
    const check = jsonschema.validate(search, companySearchSchema)
    if(!check.valid){
      throw new ExpressError('query data is invalid', 400)
    }
    // query all company based on filter terms.
